...
uv run pytest tests/integration -k grpc --robot <robot-ip>:50051

# HTTP robot-server API tests against the live Flex
uv run pytest tests/integration/http_api --robot-http <robot-ip>:31950

# Both, in one run
uv run pytest tests/integration --robot <robot-ip>:50051
```
