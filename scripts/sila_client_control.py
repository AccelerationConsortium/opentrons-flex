...
async def main(robot_ip: str, port: int):
    ...
    async def check_machine_status(stage: str):
        ...
        print(
            f"[{stage}] MachineStatus: estop={status.estop}, door_open={status.door_open}, "
            f"is_error_state={status.is_error_state}, message={status.message}"
        )
    ...
    try:
        ...
        if not has_tip:
            ...
            print(
                "The script will still move the gantry through all the target slots (C1, C2) "
                "to verify coordinates, but will bypass plunger commands."
            )
        ...
        print(
            f"Moving Gripper to Slot C3 center (X={SLOT_C3['x']}, Y={SLOT_C3['y']}, "
            f"Z={SAFE_Z_GANTRY}) [Speed: 50 mm/s]..."
        )
        await call_command(
            _SERVICE,
            _PKG,
        ...
        print(
            f"Moving Gripper to Slot C2 center (X={SLOT_C2['x']}, Y={SLOT_C2['y']}, "
            f"Z={SAFE_Z_GANTRY}) [Speed: 50 mm/s]..."
        )
        await call_command(
            _SERVICE,
        ...
        print(
            f"Moving Pipette to Slot C1 center (X={SLOT_C1['x']}, Y={SLOT_C1['y']}, "
            f"Z={SAFE_Z_GANTRY}) [Speed: 50 mm/s]..."
        )
        await call_command(
            _SERVICE,
        ...
        print(
            f"Moving Pipette to Slot C2 center (X={SLOT_C2['x']}, Y={SLOT_C2['y']}, "
            f"Z={SAFE_Z_GANTRY}) [Speed: 50 mm/s]..."
        )
        await call_command(
            _SERVICE,
        ...
        if has_tip:
            ...
            await call_command(
                _SERVICE, _PKG, "Dispense", {"mount": Mount.LEFT, "volume": 20.0, "rate": 1.0, "push_out": 0.0}
            )
